...
class LoadImages:
    ...
    def __init__(self, path, imgsz=640, stride=32, auto=True, transforms=None, vid_stride=1):
        if isinstance(path, np.ndarray):
            files = [path]
        elif isinstance(path, str) and Path(path).suffix == ".txt":  # *.txt file with img/vid/dir on each line
        ...
        images = [x for x in files if isinstance(x, np.ndarray) or x.split('.')[-1].lower() in IMG_FORMATS]
        videos = [x for x in files if not isinstance(x, np.ndarray) and x.split('.')[-1].lower() in VID_FORMATS]
